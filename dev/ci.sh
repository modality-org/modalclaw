...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    local build_args=(
      ...
      -t modalclaw-local-smoke:latest
      .
    )
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t modalclaw-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
ModalClaw Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm modalclaw-local-smoke:latest --version
    ;;
