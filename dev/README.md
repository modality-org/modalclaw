# ModalClaw Development Environment

A fully containerized development sandbox for ModalClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`modalclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `modalclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/modalclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.modalclaw` (mounted to `/modalclaw-data/.modalclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.modalclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `modalclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=modalclaw-target`) to speed repeat local image builds.
