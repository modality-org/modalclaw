#!/bin/bash
# ModalClaw Telegram Integration Test Suite
...
if [ -f "target/release/modalclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/modalclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/modalclaw 2>/dev/null || stat -c%s target/release/modalclaw)
...
CONFIG_PATH="$HOME/.modalclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'modalclaw onboard' first"
    fi
else
    warn "No config file found - run 'modalclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/modalclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
