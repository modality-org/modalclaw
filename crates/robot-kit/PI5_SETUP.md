...
Complete guide to setting up a ModalClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.modalclaw/models
cp models/ggml-base.bin ~/.modalclaw/models/
```
...
mkdir -p ~/.modalclaw/models/piper
cd ~/.modalclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.modalclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build ModalClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/modalclaw
cd modalclaw

# Build robot kit
cargo build --release -p modalclaw-robot-kit

# Build main modalclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.modalclaw
nano ~/.modalclaw/robot.toml
```

```toml
# ~/.modalclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.modalclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/modalclaw_sensors.fifo"
...
### Start ModalClaw Agent

```bash
# Configure ModalClaw to use robot tools
cat > ~/.modalclaw/config.toml << 'EOF'
...
cp ~/modalclaw/crates/robot-kit/SOUL.md ~/.modalclaw/workspace/

# Start agent
./target/release/modalclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/modalclaw_sensors.fifo ]; then
    mkfifo /tmp/modalclaw_sensors.fifo
fi
...
# Start modalclaw
cd ~/modalclaw
./target/release/modalclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/modalclaw-robot.service
sudo tee /etc/systemd/system/modalclaw-robot.service << 'EOF'
[Unit]
Description=ModalClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/modalclaw
...
sudo systemctl enable modalclaw-robot
sudo systemctl start modalclaw-robot

# Check status
sudo systemctl status modalclaw-robot
journalctl -u modalclaw-robot -f  # View logs
```
