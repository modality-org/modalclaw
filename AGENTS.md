# AGENTS.md — ModalClaw Agent Engineering Protocol
...
ModalClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use ModalClaw-native labels only (`ModalClawAgent`, `modalclaw_user`, `modalclaw_node`).
...
- If identity-like context is unavoidable, use ModalClaw-scoped roles/labels only (for example: `ModalClawAgent`, `ModalClawOperator`, `modalclaw_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `ModalClawAgent`, `ModalClawOperator`, `ModalClawMaintainer`, `modalclaw_user`
    - service/runtime labels: `modalclaw_bot`, `modalclaw_service`, `modalclaw_runtime`, `modalclaw_node`
    - environment labels: `modalclaw_project`, `modalclaw_workspace`, `modalclaw_channel`
