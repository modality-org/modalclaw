...
usage() {
  cat <<'USAGE'
ModalClaw installer bootstrap engine

Usage:
  ./modalclaw_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds ModalClaw only (requires existing Rust toolchain).
...
Examples:
  ./modalclaw_install.sh
  ./modalclaw_install.sh --guided
  ./modalclaw_install.sh --install-system-deps --install-rust
  ./modalclaw_install.sh --prefer-prebuilt
  ./modalclaw_install.sh --prebuilt-only
  ./modalclaw_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./modalclaw_install.sh --interactive-onboard
...
Environment:
  MODALCLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  MODALCLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  MODALCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: modalclaw-bootstrap:local)
  MODALCLAW_API_KEY           Used when --api-key is not provided
  MODALCLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  MODALCLAW_MODEL             Used when --model is not provided
  MODALCLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  MODALCLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  MODALCLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${MODALCLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${MODALCLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/modality-org/modalclaw/releases/latest/download/modalclaw-${target}.tar.gz"
  temp_dir="$(mktemp -d -t modalclaw-prebuilt-XXXXXX)"
  archive_path="$temp_dir/modalclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/modalclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name modalclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable modalclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/modalclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/modalclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/modalclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "ModalClaw guided installer"
  ...
  if prompt_yes_no "Install modalclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${MODALCLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set MODALCLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set MODALCLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${MODALCLAW_DOCKER_IMAGE:-modalclaw-bootstrap:local}"
  fallback_image="ghcr.io/modality-org/modalclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.modalclaw-docker"
  else
    default_data_dir="$WORK_DIR/.modalclaw-docker"
  fi
  docker_data_dir="${MODALCLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.modalclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official ModalClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.modalclaw:/modalclaw-data/.modalclaw"
  workspace_mount="$docker_data_dir/workspace:/modalclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  MODALCLAW_API_KEY="sk-..." ./modalclaw_install.sh --docker
or run interactive:
  ./modalclaw_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/modalclaw-data \
    -e MODALCLAW_WORKSPACE=/modalclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/modality-org/modalclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${MODALCLAW_CONTAINER_CLI:-docker}"
API_KEY="${MODALCLAW_API_KEY:-}"
PROVIDER="${MODALCLAW_PROVIDER:-openrouter}"
MODEL="${MODALCLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${MODALCLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set MODALCLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t modalclaw-bootstrap-XXXXXX)"
...
Your containerized ModalClaw data is persisted under:
DONE
...
Next steps:
  ./modalclaw_install.sh --docker --interactive-onboard
  ./modalclaw_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./modalclaw_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing modalclaw to cargo bin"
...
MODALCLAW_BIN=""
if have_cmd modalclaw; then
  MODALCLAW_BIN="modalclaw"
elif [[ -x "$HOME/.cargo/bin/modalclaw" ]]; then
  MODALCLAW_BIN="$HOME/.cargo/bin/modalclaw"
elif [[ -x "$WORK_DIR/target/release/modalclaw" ]]; then
  MODALCLAW_BIN="$WORK_DIR/target/release/modalclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$MODALCLAW_BIN" ]]; then
    error "onboarding requested but modalclaw binary is not available."
    error "Run without --skip-install, or ensure modalclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$MODALCLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  MODALCLAW_API_KEY="sk-..." ./modalclaw_install.sh --onboard
or run interactive:
  ./modalclaw_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$MODALCLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
