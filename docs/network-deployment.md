# Network Deployment — ModalClaw on Raspberry Pi and Local Network

This document covers deploying ModalClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | ModalClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | ModalClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — ModalClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. ModalClaw on Raspberry Pi
...
```bash
modalclaw daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: ModalClaw polls Telegram API (outbound)
...
```bash
modalclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `modalclaw tunnel` (see tunnel docs).

3. ModalClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- ModalClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `modalclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `modalclaw daemon` service).
...
- [ ] Run `modalclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
ModalClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `modalclaw` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/modalclaw`
- Config directory: `/etc/modalclaw/`
- Log directory: `/var/log/modalclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo modalclaw service install` automatically prepares `/etc/modalclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `modalclaw` service user.

If no prior runtime state is available to migrate, create `/etc/modalclaw/config.toml` before starting the service.
...
# Check status
sudo rc-service modalclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service modalclaw start` | Start the daemon |
| `sudo rc-service modalclaw stop` | Stop the daemon |
| `sudo rc-service modalclaw status` | Check service status |
| `sudo rc-service modalclaw restart` | Restart the daemon |
| `sudo modalclaw service status` | ModalClaw status wrapper (uses `/etc/modalclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/modalclaw/access.log` |
| Errors/stderr | `/var/log/modalclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/modalclaw/error.log
```
...
sudo rc-service modalclaw stop
sudo rc-update del modalclaw default

# Remove init script
sudo modalclaw service uninstall
```
...
- The service runs as the `modalclaw:modalclaw` user (least privilege)
- Config must be at `/etc/modalclaw/config.toml` (explicit path in init script)
- If the `modalclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo modalclaw service install`
- [ ] Enable: `sudo rc-update add modalclaw default`
- [ ] Start: `sudo rc-service modalclaw start`
- [ ] Verify: `sudo rc-service modalclaw status`
- [ ] Check logs: `/var/log/modalclaw/error.log`
