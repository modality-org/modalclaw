...
This document is the canonical reference for channel configuration in ModalClaw.
...
6. **Stale runtime state**: config changed but `modalclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.modalclaw/config.toml`.
...
When running `modalclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `modalclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

ModalClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]` or `[channels_config.lark]` is present but the corresponding feature is not compiled in, `modalclaw channel list`, `modalclaw channel doctor`, and `modalclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@modalclaw:matrix.example.com"   # optional, recommended for E2EE
...
port = 6697
nickname = "modalclaw-bot"
username = "modalclaw"              # optional
channels = ["#modalclaw"]
allowed_users = ["*"]
...
- `MODALCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `MODALCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `modalclaw daemon` after config changes.
...
```bash
RUST_LOG=info modalclaw daemon 2>&1 | tee /tmp/modalclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/modalclaw.log
```
