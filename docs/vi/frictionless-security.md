...
[INFO] ✓ Audit logging enabled (~/.config/modalclaw/audit.log)
...
$ modalclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/modalclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ modalclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ modalclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ modalclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ modalclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `modalclaw security --status`
