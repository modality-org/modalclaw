...
Hướng dẫn này giải thích cách sử dụng gói Python `modalclaw-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của ModalClaw sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
```python
from modalclaw_tools import tool, create_agent
...
# Single message
modalclaw-tools "What is the current date?"

# Interactive mode
modalclaw-tools -i
```

## So sánh với Rust ModalClaw

| Khía cạnh | Rust ModalClaw | modalclaw-tools |
|--------|---------------|-----------------|
