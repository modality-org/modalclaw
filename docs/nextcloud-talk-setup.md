...
This guide covers native Nextcloud Talk integration for ModalClaw.
...
Add this section in `~/.modalclaw/config.toml`:
...
modalclaw daemon
# or
modalclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, ModalClaw verifies:
...
- ModalClaw ignores bot-originated webhook events (`actorType = bots`).
- ModalClaw ignores non-message/system events.
...
3. Confirm ModalClaw receives and replies in the same room.
