# ModalClaw on Nucleo-F401RE — Step-by-Step Guide

Run ModalClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

ModalClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
ModalClaw includes everything for Nucleo-F401RE:
...
| Flash command | `modalclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via ModalClaw

From the modalclaw repo root:

```bash
modalclaw peripheral flash-nucleo
```

This builds `firmware/modalclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `modalclaw daemon` or `modalclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `modalclaw peripheral list` to verify.
