# ModalClaw Security Improvement Roadmap
...
ModalClaw already has **excellent application-layer security**:
...
## Comparison: ModalClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | ModalClaw Now | ModalClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`modalclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/modalclaw/audit.log"
sign_events = true
max_size_mb = 100
...
modalclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
modalclaw audit --user @alice --since 24h
modalclaw audit --risk high --violations-only
modalclaw audit --verify-signatures

# Sandbox test
modalclaw sandbox --test
# → Testing isolation...
...
**ModalClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, ModalClaw becomes:
...
**Value**: Transforms ModalClaw from "safe for testing" to "safe for production"
