...
- `cargo check` / `cargo build` dừng lâu ở `Checking modalclaw`
...
### Không tìm thấy lệnh `modalclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `modalclaw`
...
Xác minh `~/.modalclaw/config.toml`:
...
- Dừng các tiến trình `modalclaw daemon` / `modalclaw channel start` thừa
