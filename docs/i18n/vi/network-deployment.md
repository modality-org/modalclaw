# Triển khai mạng — ModalClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai ModalClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | ModalClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | ModalClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — ModalClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. ModalClaw trên Raspberry Pi
...
Chỉnh sửa `~/.modalclaw/config.toml`:
...
- Channel Telegram hoạt động: ModalClaw poll Telegram API (outbound)
...
```bash
modalclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `modalclaw tunnel` (xem tài liệu tunnel).

3. ModalClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- ModalClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `modalclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `modalclaw daemon`).
...
- [ ] Chạy `modalclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
