...
|---|---|---|
| Foreground runtime | `modalclaw daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `modalclaw gateway` | kiểm thử webhook endpoint |
| User service | `modalclaw service install && modalclaw service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `modalclaw doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `modalclaw channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `modalclaw status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.modalclaw/daemon_state.json` | file được cập nhật định kỳ |

## Log và Chẩn đoán

### macOS / Windows (log của service wrapper)

- `~/.modalclaw/logs/daemon.stdout.log`
- `~/.modalclaw/logs/daemon.stderr.log`

### Linux (systemd user service)

```bash
journalctl --user -u modalclaw.service -f
```
...
1. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.modalclaw/config.toml`.
...
1. sao lưu `~/.modalclaw/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `modalclaw doctor`
