# Adding Boards and Tools — ModalClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to ModalClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.modalclaw/config.toml)
modalclaw peripheral add nucleo-f401re /dev/ttyACM0
modalclaw peripheral add arduino-uno /dev/cu.usbmodem12345
modalclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, ModalClaw can index PDF files:
...
2. **Add to config** — `modalclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `modalclaw peripheral list` | List configured boards |
| `modalclaw peripheral add <board> <path>` | Add board (writes config) |
| `modalclaw peripheral flash` | Flash Arduino firmware |
| `modalclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `modalclaw hardware discover` | List USB devices |
| `modalclaw hardware info` | Chip info via probe-rs |
