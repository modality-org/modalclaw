...
3. Generic fallback env vars: `MODALCLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, ModalClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, ModalClaw returns a structured capability error instead of silently ignoring images.
...
- ModalClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `modalclaw models refresh --provider llamacpp`
...
- Model discovery: `modalclaw models refresh --provider sglang`
...
- Model discovery: `modalclaw models refresh --provider vllm`
...
- Model discovery: `modalclaw models refresh --provider osaurus`
...
- Model discovery: `modalclaw models refresh --provider nvidia`
