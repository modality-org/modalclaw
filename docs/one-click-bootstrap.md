...
This page defines the fastest supported path to install and initialize ModalClaw.
...
Default behavior is **app-only** (build/install ModalClaw) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/modality-org/modalclaw/main/scripts/install.sh | bash
```
...
This builds a local ModalClaw image and launches onboarding inside a container while
persisting config/workspace to `./.modalclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `MODALCLAW_CONTAINER_CLI`
explicitly (for example: `MODALCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`MODALCLAW_DOCKER_IMAGE`, default: `modalclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/modality-org/modalclaw:latest` and tags it locally before running.
...
```bash
MODALCLAW_API_KEY="sk-..." MODALCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/modality-org/modalclaw:latest`)
- `--skip-install`
- `--provider <id>`
