# ModalClaw Config Reference (Operator-Oriented)
...
1. `MODALCLAW_WORKSPACE` override (if set)
2. persisted `~/.modalclaw/active_workspace.toml` marker (if present)
3. default `~/.modalclaw/config.toml`

ModalClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `modalclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `modalclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `modalclaw doctor traces --limit 20`
  - `modalclaw doctor traces --event tool_call_result --contains \"error\"`
  - `modalclaw doctor traces --id <trace-id>`
...
1. `MODALCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `MODALCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, ModalClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.modalclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `modalclaw estop` to engage and `modalclaw estop resume` to clear levels.
...
- Security-first default: ModalClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `MODALCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `MODALCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `MODALCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `MODALCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `modalclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- ModalClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `modalclaw doctor` before restart/rollout.
...
- While `modalclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `MODALCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `MODALCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
