...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `modalclaw onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `modalclaw onboard --interactive` |
| Đã có config, chỉ cần sửa kênh | `modalclaw onboard --channels-only` |
...
- Thiết lập nhanh: `modalclaw onboard --api-key "sk-..." --provider openrouter`
- Thiết lập tương tác: `modalclaw onboard --interactive`
- Kiểm tra môi trường: `modalclaw status` + `modalclaw doctor`
