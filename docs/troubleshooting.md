...
- `cargo check` / `cargo build` appears stuck at `Checking modalclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in ModalClaw:
...
### `modalclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `modalclaw`
...
- stop extra `modalclaw daemon` / `modalclaw channel start` processes
