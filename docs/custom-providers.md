...
ModalClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export MODALCLAW_API_KEY="your-api-key"
modalclaw agent
```

## llama.cpp Server (Recommended Local Setup)

ModalClaw includes a first-class local provider for `llama-server`:
...
```bash
modalclaw models refresh --provider llamacpp
modalclaw agent -m "hello"
```

You do not need to export `MODALCLAW_API_KEY=dummy` for this flow.

## SGLang Server

ModalClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
modalclaw models refresh --provider sglang
modalclaw agent -m "hello"
```
...
ModalClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
modalclaw models refresh --provider vllm
modalclaw agent -m "hello"
```
...
# Single message test
modalclaw agent -m "test message"
```
