...
- **Audience:** operators and maintainers running ModalClaw in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside ModalClaw runtime behavior.
...
### 1.1 Proxy only ModalClaw internal traffic

1. Use scope `modalclaw`.
2. Set `http_proxy`/`https_proxy` or `all_proxy`.
3. Validate with `{"action":"get"}`.

Go to:

- [Section 4](#4-mode-a--proxy-only-for-modalclaw-internals)
...
|---|---|---|---|
| `modalclaw` | ModalClaw internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for ModalClaw Internals

Use when ModalClaw provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.
...
- Runtime proxy is active for ModalClaw HTTP clients.
