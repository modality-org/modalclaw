...
/// Tokens are cached to `~/.config/modalclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "modalclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("modalclaw-copilot-{user}"))
            });
