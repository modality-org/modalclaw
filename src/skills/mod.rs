...
const OPEN_SKILLS_SYNC_MARKER: &str = ".modalclaw-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.modalclaw/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("MODALCLAW_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("MODALCLAW_OPEN_SKILLS_DIR").ok();
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.modalclaw/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.modalclaw/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: modalclaw skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.modalclaw");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.modalclaw/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("MODALCLAW_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("MODALCLAW_OPEN_SKILLS_DIR");
