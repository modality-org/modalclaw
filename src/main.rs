...
/// `ModalClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "modalclaw")]
...
Examples:
  modalclaw agent                              # interactive session
  modalclaw agent -m \"Summarize today's logs\"  # single message
  modalclaw agent -p anthropic --model claude-sonnet-4-20250514
  modalclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  modalclaw gateway                  # use config defaults
  modalclaw gateway -p 8080          # listen on port 8080
  modalclaw gateway --host 0.0.0.0   # bind to all interfaces
  modalclaw gateway -p 0             # random available port")]
    Gateway {
...
Launches the full ModalClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run ModalClaw in \
production or as an always-on assistant.

Use 'modalclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  modalclaw daemon                   # use config defaults
  modalclaw daemon -p 9090           # gateway on port 9090
  modalclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `modalclaw estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `modalclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  modalclaw cron list
  modalclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  modalclaw cron add '*/30 * * * *' 'Check system health'
  modalclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  modalclaw cron add-every 60000 'Ping heartbeat'
  modalclaw cron once 30m 'Run backup in 30 minutes'
  modalclaw cron pause <task-id>
  modalclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect ModalClaw \
...
Examples:
  ...
  modalclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  modalclaw channel remove my-bot
  modalclaw channel bind-telegram modalclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  modalclaw hardware introspect /dev/ttyACM0
  modalclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: modalclaw::HardwareCommands,
    },
...
Examples:
  ...
  modalclaw peripheral add nucleo-f401re /dev/ttyACM0
  modalclaw peripheral add rpi-gpio native
  modalclaw peripheral flash --port /dev/cu.usbmodem12345
  modalclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: modalclaw::PeripheralCommands,
    },
...
Examples:
  modalclaw memory stats
  modalclaw memory list
  modalclaw memory list --category core --limit 10
  modalclaw memory get <key>
  modalclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage ModalClaw configuration.
...
Examples:
  modalclaw config schema              # print JSON Schema to stdout
  modalclaw config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `modalclaw`.
...
Examples:
  ...
  modalclaw completions fish > ~/.config/fish/completions/modalclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("MODALCLAW_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("MODALCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for ModalClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting ModalClaw Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting ModalClaw Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting ModalClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting ModalClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 ModalClaw Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `modalclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `modalclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `modalclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `modalclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `modalclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `modalclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["modalclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["modalclaw", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["modalclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["modalclaw", "estop", "resume", "--domain", "*.chase.com"])
