...
// in `~/.modalclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.modalclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(modalclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: modalclaw_dir.join(".secret_key"),
            enabled,
        }
    }
