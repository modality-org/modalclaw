...
const SERVICE_LABEL: &str = "com.modalclaw.daemon";
const WINDOWS_TASK_NAME: &str = "ModalClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "modalclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["modalclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "modalclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["modalclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "modalclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["modalclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["modalclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/modalclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/modalclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "modalclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove modalclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/modalclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: modalclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=ModalClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "modalclaw.service"]));
...
/// Check if the modalclaw user exists and has expected properties.
...
fn check_modalclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "modalclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser modalclaw && delgroup modalclaw",
            "addgroup -S modalclaw && adduser -S -s /sbin/nologin -H -D -G modalclaw modalclaw",
        )
    } else {
        ("userdel modalclaw", "useradd -r -s /sbin/nologin modalclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'modalclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'modalclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/modalclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: modalclaw user has home directory '{}' (expected /var/lib/modalclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_modalclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "modalclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_modalclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "modalclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "modalclaw"])
                .output()
                .context("Failed to create modalclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create modalclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: modalclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create modalclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create modalclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "modalclaw"])
            .output()
            .context("Failed to create modalclaw user")?;
    ...
    println!("✅ Created system user: modalclaw");
    Ok(())
}

/// Change ownership of a path to modalclaw:modalclaw
#[cfg(unix)]
fn chown_to_modalclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["modalclaw:modalclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to modalclaw:modalclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_modalclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_modalclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "modalclaw:modalclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to modalclaw:modalclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_modalclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".modalclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'modalclaw' cannot write {} ({details}). \
             Re-run `sudo modalclaw service install` and ensure ownership is modalclaw:modalclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/modalclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="modalclaw:modalclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/modalclaw/access.log"
error_log="/var/log/modalclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/modalclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo modalclaw service install"
        );
    }

    ensure_modalclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/modalclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/modalclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_modalclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by modalclaw:modalclaw)",
    ...
    chown_recursive_to_modalclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/modalclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "modalclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/modalclaw");
    println!("   Config path: /etc/modalclaw/config.toml");
    println!("   Start with: sudo modalclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("modalclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/modalclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "ModalClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/modalclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/modalclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"modalclaw\""));
        assert!(script.contains("description=\"ModalClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/modalclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/modalclaw daemon\""));
        assert!(!script.contains("env MODALCLAW_CONFIG_DIR"));
        assert!(!script.contains("env MODALCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"modalclaw:modalclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/modalclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/modalclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/modalclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/modalclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/modalclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/modalclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/modalclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/modalclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/modalclaw/workspace'".to_string(),
                "modalclaw".to_string()
            ]
        );
    }
