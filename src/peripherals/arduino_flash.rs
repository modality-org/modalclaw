//! Flash ModalClaw Arduino firmware via arduino-cli.
//!
...
/// ModalClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/modalclaw-arduino/modalclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "modalclaw-arduino";
...
/// Flash ModalClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("modalclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling ModalClaw Arduino firmware...");
    ...
    println!("ModalClaw firmware flashed successfully.");
