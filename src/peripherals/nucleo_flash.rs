//! Flash ModalClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash ModalClaw Nucleo firmware. Builds from firmware/modalclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (modalclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("modalclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from modalclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building ModalClaw Nucleo firmware...");
    ...
    println!("ModalClaw Nucleo firmware flashed successfully.");
