...
Examples:
  ...
  modalclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `ModalClaw` workspace
    Openclaw {
...
Examples:
  ...
  modalclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  modalclaw cron add-every 60000 'Ping heartbeat'     # every minute
  modalclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  modalclaw cron once 2h 'Follow up on deployment'
  modalclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  modalclaw cron update <task-id> --expression '0 8 * * *'
  modalclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  modalclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  modalclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  modalclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash ModalClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash ModalClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  modalclaw peripheral flash
  modalclaw peripheral flash --port /dev/cu.usbmodem12345
  modalclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash ModalClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
