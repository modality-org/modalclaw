...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/modalclaw/security/advisories/new)
...
ModalClaw implements defense-in-depth security:
...
ModalClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t modalclaw .
docker inspect --format='{{.Config.User}}' modalclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace modalclaw gateway
```
