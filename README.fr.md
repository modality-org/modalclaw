...
<p align="center">
  ModalClaw est le <strong>système d'exploitation runtime</strong> pour les workflows agentiques — une infrastructure qui abstrait les modèles, outils, mémoire et exécution pour construire des agents une fois et les exécuter partout.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/modalclaw` ou `modalclaw.org`. Le domaine `modalclaw.org` pointe actuellement vers le fork `openagen/modalclaw`, et ce domaine/dépôt usurpe l'identité de notre site web/projet officiel.                                                                                                                                                                                 | Ne faites pas confiance aux informations, binaires, levées de fonds ou annonces provenant de ces sources. Utilisez uniquement [ce dépôt](https://github.com/modality-org/modalclaw) et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Important_ | Notre site officiel est désormais en ligne : [modalclawlabs.ai](https://modalclawlabs.ai). Merci pour votre patience pendant cette attente. Nous constatons toujours des tentatives d'usurpation : ne participez à aucune activité d'investissement/financement au nom de ModalClaw si elle n'est pas publiée via nos canaux officiels.                                                                                                                   | Utilisez [ce dépôt](https://github.com/modality-org/modalclaw) comme source unique de vérité. Suivez [X (@modalclawlabs)](https://x.com/modalclawlabs?s=21), [Reddit (r/modalclawlabs)](https://www.reddit.com/r/modalclawlabs/), [Telegram (@modalclawlabs)](https://t.me/modalclawlabs), [Telegram CN (@modalclawlabs_cn)](https://t.me/modalclawlabs_cn), [Telegram RU (@modalclawlabs_ru)](https://t.me/modalclawlabs_ru), et [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) pour les mises à jour officielles. |
...
### Pourquoi les équipes choisissent ModalClaw
...
## Instantané de Benchmark (ModalClaw vs OpenClaw, Reproductible)
...
- `modalclaw --help` : environ `0.02s` de temps réel, ~`3.9 Mo` d'empreinte mémoire maximale
- `modalclaw status` : environ `0.01s` de temps réel, ~`4.1 Mo` d'empreinte mémoire maximale
...
Le script `bootstrap.sh` installe Rust, clone ModalClaw, le compile, et configure votre environnement de développement initial :

```bash
curl -fsSL https://raw.githubusercontent.com/modality-org/modalclaw/main/bootstrap.sh | bash
```

Ceci va :

1. Installer Rust (si absent)
2. Cloner le dépôt ModalClaw
3. Compiler ModalClaw en mode release
4. Installer `modalclaw` dans `~/.cargo/bin/`
5. Créer la structure de workspace par défaut dans `~/.modalclaw/workspace/`
6. Générer un fichier de configuration `~/.modalclaw/workspace/config.toml` de démarrage

Après le bootstrap, relancez votre shell ou exécutez `source ~/.cargo/env` pour utiliser la commande `modalclaw` globalement.
...
1. Configurez vos fournisseurs d'IA dans `~/.modalclaw/workspace/config.toml`
2. Consultez la [référence de configuration](docs/config-reference.md) pour les options avancées
3. Lancez l'agent : `modalclaw agent start`
4. Testez via votre canal préféré (voir [référence des canaux](docs/channels-reference.md))

## Configuration

Éditez `~/.modalclaw/workspace/config.toml` pour configurer les fournisseurs, canaux et comportement du système.
...
ModalClaw prend en charge deux backends d'exécution de code :
...
modalclaw init                # Initialise un nouveau workspace
modalclaw status              # Affiche l'état du daemon/agent
modalclaw config validate     # Vérifie la syntaxe et les valeurs de config.toml

# Gestion du daemon
modalclaw daemon start        # Démarre le daemon en arrière-plan
modalclaw daemon stop         # Arrête le daemon en cours d'exécution
modalclaw daemon restart      # Redémarre le daemon (rechargement de config)
modalclaw daemon logs         # Affiche les journaux du daemon

# Gestion de l'agent
modalclaw agent start         # Démarre l'agent (nécessite daemon en cours d'exécution)
modalclaw agent stop          # Arrête l'agent
modalclaw agent restart       # Redémarre l'agent (rechargement de config)

# Opérations de pairing
modalclaw pairing init        # Génère un nouveau secret de pairing
modalclaw pairing rotate      # Fait tourner le secret de pairing existant

# Tunneling (pour exposition publique)
modalclaw tunnel start        # Démarre un tunnel vers le daemon local
modalclaw tunnel stop         # Arrête le tunnel actif

# Diagnostic
modalclaw doctor              # Exécute les vérifications de santé du système
modalclaw version             # Affiche la version et les informations de build
```
...
enabled = true
homeserver_url = "https://matrix.org"
username = "@modalclaw:matrix.org"
password = "..."
device_name = "modalclaw-prod"
e2ee_enabled = true
```

Invitez `@modalclaw:matrix.org` dans une salle chiffrée, et le bot répondra avec le chiffrement complet. Voir [Guide Matrix E2EE](docs/matrix-e2ee-guide.md) pour la configuration de vérification de dispositif.
...
[memory]
kind = "sqlite"
path = "~/.modalclaw/workspace/memory/conversations.db"
...
[memory]
kind = "markdown"
path = "~/.modalclaw/workspace/memory/"
```
...
ModalClaw prend en charge les endpoints compatibles OpenAI :
...
ModalClaw fournit des outils intégrés pour l'exécution de code, l'accès au système de fichiers et la récupération web :
...
```bash
modalclaw daemon start
modalclaw agent start
```
...
sudo cp deployment/systemd/modalclaw-daemon.service /etc/systemd/system/
sudo cp deployment/systemd/modalclaw-agent.service /etc/systemd/system/

# Activez et démarrez les services
sudo systemctl enable modalclaw-daemon modalclaw-agent
sudo systemctl start modalclaw-daemon modalclaw-agent

# Vérifiez le statut
sudo systemctl status modalclaw-daemon
sudo systemctl status modalclaw-agent
```
...
```bash
# Compilez l'image
docker build -t modalclaw:latest .

# Exécutez le conteneur
docker run -d \
  --name modalclaw \
  -v ~/.modalclaw/workspace:/workspace \
...
ModalClaw est conçu pour fonctionner sur du matériel à faible consommation d'énergie :
...
Exposez votre daemon ModalClaw local au réseau public via des tunnels sécurisés :

```bash
modalclaw tunnel start --provider cloudflare
```
...
ModalClaw implémente plusieurs couches de sécurité :

### Pairing

Le daemon génère un secret de pairing au premier lancement stocké dans `~/.modalclaw/workspace/.pairing`. Les clients (agent, CLI) doivent présenter ce secret pour se connecter.

```bash
modalclaw pairing rotate  # Génère un nouveau secret et invalide l'ancien
```
...
ModalClaw journalise vers `~/.modalclaw/workspace/logs/` par défaut. Les journaux sont stockés par composant :

```
~/.modalclaw/workspace/logs/
...
Support de métriques Prometheus pour la surveillance en production à venir. Suivi dans [#234](https://github.com/modality-org/modalclaw/issues/234).

## Compétences (Skills)

ModalClaw prend en charge les compétences personnalisées — des modules réutilisables qui étendent les capacités du système.

### Définition de Compétence

Les compétences sont stockées dans `~/.modalclaw/workspace/skills/<nom-compétence>/` avec cette structure :
...
ModalClaw prend en charge les [Open Skills](https://github.com/openagents-com/open-skills) — un système modulaire et agnostique des fournisseurs pour étendre les capacités des agents IA.
...
Vous pouvez également surcharger au runtime avec `MODALCLAW_OPEN_SKILLS_ENABLED` et `MODALCLAW_OPEN_SKILLS_DIR`.
...
ModalClaw est configuré pour utiliser `rustls` pour les dépendances HTTP/TLS ; `--locked` maintient le graphe transitif déterministe sur les environnements vierges.
...
**Ceci est le seul dépôt officiel ModalClaw :**

> <https://github.com/modality-org/modalclaw>

Tout autre dépôt, organisation, domaine ou package prétendant être "ModalClaw" ou impliquant une affiliation avec ModalClaw Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](TRADEMARK.md).

Si vous rencontrez une usurpation d'identité ou une utilisation abusive de marque, veuillez [ouvrir une issue](https://github.com/modality-org/modalclaw/issues).

---

## Licence

ModalClaw est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom **ModalClaw** et le logo sont des marques déposées de ModalClaw Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer une approbation ou une affiliation. Voir [TRADEMARK.md](TRADEMARK.md) pour les utilisations permises et interdites.
...
- Nouvelle `Skill` → `~/.modalclaw/workspace/skills/<n>/`

---

**ModalClaw** — Zéro surcharge. Zéro compromis. Déployez n'importe où. Échangez n'importe quoi. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#modality-org/modalclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Graphique Historique des Étoiles" src="https://api.star-history.com/svg?repos=modality-org/modalclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
