...
| 日期（UTC） | 级别 | 通知 | 处理建议 |
|---|---|---|---|
| 2026-02-19 | _紧急_ | 我们与 `openagen/modalclaw` 及 `modalclaw.org` **没有任何关系**。`modalclaw.org` 当前会指向 `openagen/modalclaw` 这个 fork，并且该域名/仓库正在冒充我们的官网与官方项目。 | 请不要相信上述来源发布的任何信息、二进制、募资活动或官方声明。请仅以[本仓库](https://github.com/modality-org/modalclaw)和已验证官方社媒为准。 |
| 2026-02-21 | _重要_ | 我们的官网现已上线：[modalclawlabs.ai](https://modalclawlabs.ai)。感谢大家一直以来的耐心等待。我们仍在持续发现冒充行为，请勿参与任何未经我们官方渠道发布、但打着 ModalClaw 名义进行的投资、募资或类似活动。 | 一切信息请以[本仓库](https://github.com/modality-org/modalclaw)为准；也可关注 [X（@modalclawlabs）](https://x.com/modalclawlabs?s=21)、[Reddit（r/modalclawlabs）](https://www.reddit.com/r/modalclawlabs/)、[Telegram（@modalclawlabs）](https://t.me/modalclawlabs)、[Telegram 中文频道（@modalclawlabs_cn）](https://t.me/modalclawlabs_cn)、[Telegram 俄语频道（@modalclawlabs_ru）](https://t.me/modalclawlabs_ru) 与 [小红书账号](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) 获取官方最新动态。 |
...
ModalClaw 是一个高性能、低资源占用、可组合的自主智能体运行时。ModalClaw 是面向智能代理工作流的**运行时操作系统** — 它抽象了模型、工具、记忆和执行层，使代理可以一次构建、随处运行。
...
## 基准快照（ModalClaw vs OpenClaw，可复现）
...
- Release 二进制：`8.8M`
- `modalclaw --help`：约 `0.02s`，峰值内存约 `3.9MB`
- `modalclaw status`：约 `0.01s`，峰值内存约 `4.1MB`
...
ModalClaw 现已支持基于订阅的原生鉴权配置（多账号、静态加密存储）。

- 配置文件：`~/.modalclaw/auth-profiles.json`
- 加密密钥：`~/.modalclaw/.secret_key`
...
<p align="center">
  <img src="docs/architecture.svg" alt="ModalClaw 架构图" width="900" />
</p>

| 子系统 | Trait | 内置实现 | 扩展方式 |
|--------|-------|----------|----------|
| **AI 模型** | `Provider` | 通过 `modalclaw providers` 查看（当前 28 个内置 + 别名，以及自定义端点） | `custom:https://your-api.com`（OpenAI 兼容）或 `anthropic-custom:https://your-api.com` |
...
配置了不支持的 `runtime.kind` 时，ModalClaw 会以明确的错误退出，而非静默回退到 native。
